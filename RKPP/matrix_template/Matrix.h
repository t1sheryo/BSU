...
private:
    ...
    int rankOfMatrix(){
        std::vector<std::vector<T>> mat(matr);
        int rank = mat[0].size();
        int rows = mat.size();
        for (int row = 0; row < rank; row++){
            if (mat[row][row] != (T)0){
                for (int col = 0; col < rows; col++){
                    if (col != row){
                        T mult = (T)mat[col][row] / mat[row][row];
                        for (int i = 0; i < rank; i++) {
                            T var = mult * mat[row][i];
                            mat[col][i] -= var;
                        }
                    }
                }
...
public:
    ...
    T det() const{
        ...
        if (matr.size() == 1) {
            ...
            for(size_t j = 0; j < matr[0].size(); ++j){
                ...
                int deg = j + 1 + 1;
                res += submatr.det() * matr[0][j] * pow(-1, deg);
            }
        }
        return res;
    }
    ...
    Matrix<M, M, T> inverted() const{
        T det = this->det();
        if(det == (T)0) throw "Cannot find inverted matrix because det = 0!\n";
        Matrix<M, N, T> ret;
