...
public:
    ...
    void setOwnerName(std::string a) { ownerName = a; }
        ...
        for(auto& owner : ownerAnimals){
            for(auto& petType : owner.second){
                ...
                for(auto& petName : petType.second){
                    currentPet->setOwnerName(owner.first);
                    currentPet->setType(petType.first);
                    currentPet->setName(petName);
                    currentPet->setAge(typeAges[petType.first][petName]);

                    cout << "Owner: " << currentPet->getOwnerName() << ", Pet Type: " << currentPet->getType() << ", Pet Name: "<< currentPet-> getname() << ", Age: " << currentPet->getAge() << endl;
        ...
        while(true){
            ...
            switch(choice){
                ...
                case 5:
                    exit(1);
                default:
                    cout << "Wrong input number, it should be from 1 to 5\nIf you want to continue, press 0\n";
                    int cnt;
                    std::cin >> cnt;
                    if(cnt == 0)
                        goto point;
